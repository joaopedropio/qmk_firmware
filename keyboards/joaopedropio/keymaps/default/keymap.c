...
//             if (on) {
//                 qp_setpixel(lcd, i, j, 255, 255, 255);
//             } else {
//                 qp_setpixel(lcd, i, j, 0, 0, 0);
//             }
//         }
//     }
//     qp_flush(lcd);
...
static uint8_t get_next_color(void) {
    if (next_color > 255) {
        next_color = 0;
    }
    return next_color++;
...
static void snake_render_pixel(uint8_t x, uint8_t y, bool on) {
    uint8_t initial_y = y * SNAKE_PIXEL_SIZE;
    uint8_t initial_x = x * SNAKE_PIXEL_SIZE;
    uint8_t color = get_next_color();
    if (on) {
        qp_rect(lcd, initial_x, initial_y, initial_x + SNAKE_PIXEL_SIZE, initial_y + SNAKE_PIXEL_SIZE, color, 255, 255, true);
    } else {
        qp_rect(lcd, initial_x, initial_y, initial_x + SNAKE_PIXEL_SIZE, initial_y + SNAKE_PIXEL_SIZE, 0, 0, 0, true);
    }
    qp_flush(lcd);
}
...
//     if (timer_elapsed32(last_draw) > 6) {
